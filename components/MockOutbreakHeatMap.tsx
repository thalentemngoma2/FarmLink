...
export default function MockOutbreakHeatMap({ initialFilter = 'all' }: MockOutbreakHeatMapProps) {
  const [filter, setFilter] = useState<'all' | 'cow' | 'chicken' | 'goat'>(initialFilter);
  const [selectedOutbreak, setSelectedOutbreak] = useState<MockOutbreakPoint | null>(null);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.filterContainer}>
        ...
        >
          <Text style={[styles.filterText, filter === 'cow' && styles.filterTextActive]}>🐄 Cow</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[styles.filterButton, filter === 'chicken' && styles.filterActive]}
          onPress={() => setFilter('chicken')}
        >
          <Text style={[styles.filterText, filter === 'chicken' && styles.filterTextActive]}>🐔 Chicken</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[styles.filterButton, filter === 'goat' && styles.filterActive]}
          onPress={() => setFilter('goat')}
        >
          <Text style={[styles.filterText, filter === 'goat' && styles.filterTextActive]}>🐐 Goat</Text>
        </TouchableOpacity>
      </View>
          ...
          <View style={styles.modalCard}>
            ...
            {selectedOutbreak && (
              <>
                ...
                <Text style={styles.modalTitle}>{selectedOutbreak.disease_name}</Text>
                <Text style={styles.modalSubtitle}>{selectedOutbreak.location_name}</Text>
                <View style={styles.modalSeverity}>
                  <Text style={styles.modalSeverityLabel}>Severity:</Text>
                  <Text style={[styles.modalSeverityValue, { color: getSeverityColor(selectedOutbreak.severity) }]}>
                    {selectedOutbreak.severity}/5
                  </Text>
                </View>
                <Text style={styles.modalDate}>Reported: {formatDate(selectedOutbreak.reported_at)}</Text>
      ...
      <View style={styles.demoBanner}>
        ...
        <Text style={styles.demoBannerText}>Demo data – shows how outbreak heat map will work</Text>
      </View>
    </View>
  );
}
