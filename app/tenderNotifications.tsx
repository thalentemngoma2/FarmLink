...
function OutbreakReport({
  ...
  const validateEvidenceDate = async (
  ...
  const handleSubmit = async () => {
    ...
    if (!animalType || !diseaseName || !affectedCount) {
      Alert.alert(
        "Missing fields",
        "Please fill in animal type, disease, and number of affected animals.",
      );
      return;
    }
    if (mediaItems.length === 0) {
      Alert.alert(
        "Evidence required",
        "Add at least one photo or video (camera or gallery).",
      );
      return;
    }
    if (reportCountToday >= MAX_REPORTS_PER_DAY) {
      Alert.alert(
        "Daily limit",
        `You have already submitted ${MAX_REPORTS_PER_DAY} reports today.`,
      );
      return;
    }
    if (!location) {
      Alert.alert(
        "Location pending",
        "Please wait while we determine your location, or press Retry.",
      );
      return;
    }
    ...
    try {
      ...
      try {
        await supabase.rpc("update_outbreak_confidence", {
          report_uuid: data.id,
        });
        await supabase.rpc("create_outbreak_notifications", {
          report_uuid: data.id,
        });
      } catch (rpcError) {
  ...
  return (
    ...
    >
      ...
      <View style={[reportStyles.card, { marginTop: 20 }]}>
        ...
        <View style={reportStyles.mediaRow}>
          ...
          {mediaItems.length < MAX_MEDIA && (
            <TouchableOpacity
              style={reportStyles.addMediaButton}
        ...
        {mediaItems.length === 0 && (
          <Text style={reportStyles.cardErrorText}>
      ...
      <View style={[reportStyles.card, { flexDirection: "row" }]}>
        ...
        >
          You have submitted {reportCountToday} out of {MAX_REPORTS_PER_DAY}{" "}
          reports today.
        </Text>
      </View>
      ...
      <TouchableOpacity
        style={[
          ...
          Platform.OS === "web"
            ? reportStyles.submitButtonShadowWeb
            : reportStyles.submitButtonShadowNative,
          (submitting ||
            reportCountToday >= MAX_REPORTS_PER_DAY ||
            !location) &&
...
const styles = StyleSheet.create({
  ...
  unreadCard: {
    ...
    borderColor: "#22c55e",
    ...Platform.select({
      web: { boxShadow: "0px 0px 4px rgba(34,197,94,0.2)" },
      default: {
        shadowColor: "#22c55e",
