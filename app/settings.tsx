  ...
  const savePrivacySettings = async (newSettings: PrivacySettings) => {
    ...
    try {
      ...
      if (err.code === "PGRST204" || err.code === "42703") {
        ...
        Alert.alert(
          "Error",
          "Could not save settings. Please check your connection.",
        );
      }
    } finally {
      setSaving(false);
      ...
      >
        ...
        <Animated.View entering={SlideInDown.delay(50)} style={styles.section}>
          ...
          <GlassCard style={styles.glassCard}>
            <ToggleRow
              label="Show Username"
              value={privacy.show_username}
              onToggle={(val) => updateAndSave({ show_username: val })}
            />
            <ToggleRow
              label="Show Email"
              value={privacy.show_email}
              onToggle={(val) => updateAndSave({ show_email: val })}
            />
            <ToggleRow
              label="Show Phone"
              value={privacy.show_phone}
              onToggle={(val) => updateAndSave({ show_phone: val })}
            />
            <ToggleRow
              label="Show Location"
              value={privacy.show_location}
              onToggle={(val) => updateAndSave({ show_location: val })}
            />
            <NavRow
              ...
              description={
                privacy.profile_visibility === "public"
                  ? "Everyone"
                  : privacy.profile_visibility === "farmers_only"
                  ...
                  [
                    {
                      text: "Public",
                      onPress: () =>
                        updateAndSave({ profile_visibility: "public" }),
                    },
                    {
                      text: "Farmers Only",
                      onPress: () =>
                        updateAndSave({ profile_visibility: "farmers_only" }),
                    },
                    {
                      text: "Private",
                      onPress: () =>
                        updateAndSave({ profile_visibility: "private" }),
                    },
        ...
        <Animated.View entering={SlideInDown.delay(100)} style={styles.section}>
          ...
          <GlassCard style={styles.glassCard}>
            <NavRow
              ...
              description={
                privacy.post_visibility === "public"
                  ? "Public"
                  : privacy.post_visibility === "farmers_only"
                    ? "Farmers Only"
                    : "Groups"
              }
              onPress={() => {
                Alert.alert("Post Visibility", undefined, [
                  {
                    text: "Public",
                    onPress: () => updateAndSave({ post_visibility: "public" }),
                  },
                  {
                    text: "Farmers Only",
                    onPress: () =>
                      updateAndSave({ post_visibility: "farmers_only" }),
                  },
                  {
                    text: "Groups",
                    onPress: () => updateAndSave({ post_visibility: "groups" }),
                  },
                  { text: "Cancel", style: "cancel" },
                ]);
              }}
            ...
            <NavRow
              ...
              description={
                privacy.comment_permissions === "everyone"
                  ? "Everyone"
                  : privacy.comment_permissions === "followers"
                    ? "Followers Only"
                    : "Disabled"
              }
              onPress={() => {
                Alert.alert("Comment Permissions", undefined, [
                  {
                    text: "Everyone",
                    onPress: () =>
                      updateAndSave({ comment_permissions: "everyone" }),
                  },
                  {
                    text: "Followers Only",
                    onPress: () =>
                      updateAndSave({ comment_permissions: "followers" }),
                  },
                  {
                    text: "Disabled",
                    onPress: () =>
                      updateAndSave({ comment_permissions: "disabled" }),
                  },
                  { text: "Cancel", style: "cancel" },
                ]);
              }}
            />
            <ToggleRow
              label="Show Media (Images/Videos)"
              value={privacy.media_visibility}
              onToggle={(val) => updateAndSave({ media_visibility: val })}
            />
          </GlassCard>
        </Animated.View>
        ...
        <Animated.View entering={SlideInDown.delay(150)} style={styles.section}>
          ...
          <GlassCard style={styles.glassCard}>
            <NavRow
              ...
              description={
                privacy.message_receive_from === "everyone"
                  ? "Everyone"
                  : privacy.message_receive_from === "verified"
                    ? "Verified Users"
                    : "No One"
              }
              onPress={() => {
                Alert.alert("Message Permissions", undefined, [
                  {
                    text: "Everyone",
                    onPress: () =>
                      updateAndSave({ message_receive_from: "everyone" }),
                  },
                  {
                    text: "Verified Only",
                    onPress: () =>
                      updateAndSave({ message_receive_from: "verified" }),
                  },
                  {
                    text: "No One",
                    onPress: () =>
                      updateAndSave({ message_receive_from: "none" }),
                  },
                  { text: "Cancel", style: "cancel" },
                ]);
              }}
            ...
            <NavRow
              label="Blocked Users"
              onPress={() => router.push("/settings/privacy/blocked" as any)}
            />
            <NavRow
              label="Report a User"
        ...
        <Animated.View entering={SlideInDown.delay(200)} style={styles.section}>
          ...
          <GlassCard style={styles.glassCard}>
            <ToggleRow
              ...
              onToggle={(val) => updateAndSave({ location_sharing: val })}
            />
            <NavRow
              label="Location Precision"
              description={
                privacy.location_precision === "exact" ? "Exact" : "Approximate"
              }
              onPress={() => {
                Alert.alert(
                  "Location Precision",
                  ...
                  [
                    {
                      text: "Exact",
                      onPress: () =>
                        updateAndSave({ location_precision: "exact" }),
                    },
                    {
                      text: "Approximate",
                      onPress: () =>
                        updateAndSave({ location_precision: "approximate" }),
                    },
                    { text: "Cancel", style: "cancel" },
                  ],
                );
        ...
        <Animated.View entering={SlideInDown.delay(250)} style={styles.section}>
          ...
          <GlassCard style={styles.glassCard}>
            ...
            <ToggleRow
              ...
              onToggle={(val) =>
                updateAndSave({ evidence_access_restricted: val })
              }
            />
            <NavRow
              ...
              onPress={() => {
                Alert.alert(
                  ...
                  [
                    {
                      text: "7 days",
                      onPress: () => updateAndSave({ data_retention_days: 7 }),
                    },
                    {
                      text: "30 days",
                      onPress: () => updateAndSave({ data_retention_days: 30 }),
                    },
                    {
                      text: "90 days",
                      onPress: () => updateAndSave({ data_retention_days: 90 }),
                    },
                    { text: "Cancel", style: "cancel" },
                  ],
                );
        ...
        <Animated.View entering={SlideInDown.delay(300)} style={styles.section}>
          ...
          <GlassCard style={styles.glassCard}>
            <ToggleRow
              ...
              value={privacy.outbreak_alerts}
              onToggle={(val) => updateAndSave({ outbreak_alerts: val })}
            />
            <ToggleRow
            ...
            <ToggleRow
              ...
              value={privacy.message_notifications}
              onToggle={(val) => updateAndSave({ message_notifications: val })}
            />
            <ToggleRow
              label="System Updates"
              value={privacy.system_notifications}
              onToggle={(val) => updateAndSave({ system_notifications: val })}
            />
          </GlassCard>
        </Animated.View>
        ...
        <Animated.View entering={SlideInDown.delay(400)} style={styles.section}>
          ...
          <GlassCard style={styles.glassCard}>
            ...
            <InfoRow
              ...
              text="We comply with POPIA and the OWASP Top 10."
            />
            <InfoRow
              icon="people-outline"
              text="Role‑based access control is enforced."
            />
          </GlassCard>
        </Animated.View>
        ...
        <Animated.View entering={SlideInDown.delay(450)} style={styles.section}>
          ...
          <GlassCard style={styles.glassCard}>
            ...
            <NavRow
              ...
              onPress={() => router.push("/support/data-correction" as any)}
            />
            <NavRow
        ...
        <Animated.View entering={SlideInDown.delay(500)} style={styles.section}>
          ...
          <GlassCard style={styles.glassCard}>
            <NavRow
              label="Report Abuse or Misinformation"
              onPress={() => router.push("/report" as any)}
            />
            <NavRow
              label="Report Fake Disease Alert"
              onPress={() => router.push("/report/fake-alert" as any)}
            />
            <NavRow
              label="Community Moderation Support"
              onPress={() => router.push("/community/moderation" as any)}
            />
          </GlassCard>
        </Animated.View>
