...
export default function NotificationsScreen() {
  ...
  return (
    <View style={{ flex: 1, padding: 16 }}>
      <Text style={{ fontSize: 20, fontWeight: '700', marginBottom: 16 }}>Notifications</Text>
      {loading ? <ActivityIndicator /> : (
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity
              style={{ padding: 12, backgroundColor: item.read ? '#f3f4f6' : '#dcfce7', borderRadius: 12, marginBottom: 8 }}
...
function OutbreakReport({ onClose, onSuccess }: { onClose: () => void; onSuccess?: () => void }) {
  ...
  const fetchLocation = useCallback(async () => {
    ...
    try {
      if (Platform.OS === 'web') {
        ...
        setLocationError('GPS is mandatory for outbreak reports. Please use the mobile app.');
        return;
      }
  ...
  const checkAIImage = async (asset: ImagePicker.ImagePickerAsset): Promise<{ aiProbability: number; message?: string }> => {
    try {
      if (!SIGHTENGINE_API_USER || !SIGHTENGINE_API_SECRET) {
        if (isSuspiciousMedia(asset)) return { aiProbability: 0.9, message: 'Suspicious media (missing camera metadata or square image)' };
      ...
      return { aiProbability: isSuspiciousMedia(asset) ? 0.9 : 0.1, message: isSuspiciousMedia(asset) ? 'Suspicious media (metadata missing)' : undefined };
    }
  };
  ...
  const uploadEvidence = async (uri: string, type: 'image' | 'video'): Promise<string> => {
    ...
    const formData = new FormData();
    formData.append('file', { uri, name: fileName, type: type === 'image' ? 'image/jpeg' : 'video/mp4' } as any);
    ...
    try {
      ...
      const { data: report, error } = await supabase
        ...
        .insert({
          ...
          gps_verified: true,
          metadata_verified: verificationSteps.every(s => s.status === 'pass'),
            ...
            <View style={{ marginTop: 8 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                ...
                <Text style={{ fontSize: 15, fontWeight: '600', color: '#111827' }}>
                  Trust Score: {trustScore}% — {trustScore >= 90 ? 'Verified Authentic' : trustScore >= 70 ? 'Likely Authentic' : trustScore >= 50 ? 'Suspicious' : 'Rejected'}
                </Text>
              </View>
