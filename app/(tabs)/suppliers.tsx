  ...
  const fetchApplications = useCallback(async () => {
    ...
    try {
      ...
      const formattedApps: Application[] = appsData.map(a => ({
        ...
        farmerName: profileMap.get(a.farmer_id)?.full_name || userMap.get(a.farmer_id)?.username || 'Anonymous',
    ...
    <Animated.View entering={FadeInUp.delay(index * 50)} style={styles.card}>
      <View style={styles.cardHeader}>
        <View>
          ...
          {item.farmerLocation ? <Text style={styles.farmerLocation}>{item.farmerLocation}</Text> : null}
        </View>
        <View style={[styles.statusBadge, { backgroundColor: item.status === 'pending' ? '#fef3c7' : item.status === 'accepted' ? '#dcfce7' : '#fef2f2' }]}>
          <Text style={[styles.statusText, { color: item.status === 'pending' ? '#d97706' : item.status === 'accepted' ? '#166534' : '#991b1b' }]}>
      ...
      <Text style={styles.dateText}>Applied: {new Date(item.createdAt).toLocaleDateString()}</Text>
    ...
    <SafeAreaView style={styles.safeArea} edges={['top', 'left', 'right']}>
      <View style={styles.container}>
        ...
        <View style={styles.content}>
          ...
          <Text style={styles.pageSubtitle}>Manage applications from farmers for your tenders</Text>
          ...
          {loading ? (
            ...
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptySubtitle}>When farmers apply to your tenders, they will appear here.</Text>
            </View>
          ) : (
            <FlatList
              data={applications}
...
const styles = StyleSheet.create({
  ...
  pageTitle: { fontSize: 28, fontWeight: 'bold', color: '#11181C', paddingHorizontal: 16 },
  pageSubtitle: { fontSize: 14, color: '#687076', paddingHorizontal: 16, marginTop: 4, marginBottom: 16 },
  ...
  tenderTitle: { fontSize: 13, fontWeight: '600', color: '#4b5563', marginBottom: 8 },
  priceContainer: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#f0fdf4', alignSelf: 'flex-start', paddingHorizontal: 10, paddingVertical: 6, borderRadius: 12, marginBottom: 12 },
  priceText: { fontSize: 14, fontWeight: '700', color: '#166534', marginLeft: 6 },
  messageText: { fontSize: 14, color: '#374151', fontStyle: 'italic', marginBottom: 12, lineHeight: 20 },
  ...
  emptySubtitle: { fontSize: 14, color: '#687076', textAlign: 'center', marginTop: 8, lineHeight: 20 },
});