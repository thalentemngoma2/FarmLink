...
// -----------------------------------------------------------------------------
// Helper: map post – will be enriched with media & username later
// -----------------------------------------------------------------------------
...
const TikTokBottomSheet = forwardRef<BottomSheetRef, TikTokBottomSheetProps>(
  function TikTokBottomSheet({ visible, onClose, children }, ref) {
...
const CommunityPageContent = () => {
  ...
  // ---------------------------------------------------------------------------
  // Fetch posts with real usernames from public.users and media
  // ---------------------------------------------------------------------------
  const fetchPosts = useCallback(async () => {
    try {
      ...
      // Get usernames from public.users for all user_ids
      ...
      if (userIds.length > 0) {
        ...
        if (!usersError && usersData) {
          ...
          console.warn('Could not fetch usernames from users table:', usersError);
        }
      }

      // Build media array directly from posts.media_urls (text[]) and media_type
      const mediaMap: Record<string, MediaItem[]> = {};
      postsData.forEach(post => {
        if (post.media_urls && Array.isArray(post.media_urls) && post.media_urls.length > 0) {
          mediaMap[post.id] = post.media_urls.map((url: string) => ({
            url,
      ...
      console.log(`✅ Loaded ${mappedPosts.length} posts with real usernames`);
    } catch (err: any) {
  ...
  const handleLike = async (id: string) => {
    ...
    try {
      ...
      console.error('Failed to like post', _err);
      setPosts(prev => prev.map(p =>
        p.id === id
  ...
  // ---------------------------------------------------------------------------
  // Video auto‑play (only first video in media array)
  // ---------------------------------------------------------------------------
  const onViewableItemsChanged = useRef(({ viewableItems }: any) => {
    const visibleVideoItem = viewableItems.find((item: any) => 
      item.item.media && item.item.media.some((m: MediaItem) => m.type === 'video')
    );
    if (visibleVideoItem) {
      const videoId = visibleVideoItem.item.id;
  ...
  }, [bgScale, bgOpacity, bgScale2, bgOpacity2]);
  ...
  // ---------------------------------------------------------------------------
  // Render media carousel for a post (fixed: no fixed height, uses aspect ratio)
  // ---------------------------------------------------------------------------
  const renderMediaCarousel = (media: MediaItem[], postId: string) => {
    ...
    return (
      <FlatList
        ...
        keyExtractor={(_, idx) => `${postId}_media_${idx}`}
        renderItem={({ item, index }) => {
          if (item.type === 'video') {
            return (
              <Video
                ref={(ref) => {
                  if (ref && index === 0) videoRefs.current.set(postId, ref);
                  else if (!ref) videoRefs.current.delete(postId);
                }}
                source={{ uri: item.url }}
                style={{ width: width - 32, height: undefined, aspectRatio: 16 / 9 }}
                resizeMode={ResizeMode.COVER}
                shouldPlay={visibleVideoId === postId}
                isLooping
          ...
          return (
            <Image
              source={{ uri: item.url }}
              style={{ width: width - 32, height: undefined, aspectRatio: 1 }}
              resizeMode="contain"
            />
          );
        }}
  ...
  // ---------------------------------------------------------------------------
  // Render single post card (no title)
  // ---------------------------------------------------------------------------
  const renderItem = ({ item, index }: { item: Discussion; index: number }) => {
    return (
      <Animated.View entering={FadeIn.delay(index * 50)} style={styles.simpleCard}>
        {renderMediaCarousel(item.media, item.id)}
                ...
                <View style={styles.sheetPostHeader}>
                  {selectedPost.media && selectedPost.media.length > 0 && (
                    <FlatList
                      ...
                      pagingEnabled
                      showsHorizontalScrollIndicator={false}
                      keyExtractor={(_, idx) => `sheet_media_${idx}`}
                      renderItem={({ item }) => (
                        item.type === 'video' ? (
                          <Video
                            source={{ uri: item.url }}
...
const styles = StyleSheet.create({
  ...
  authorName: { fontSize: 15, fontWeight: '600', color: '#11181C' },
  officialBadge: { backgroundColor: '#22c55e', paddingHorizontal: 6, paddingVertical: 2, borderRadius: 12, fontSize: 10, fontWeight: '600', color: 'white', overflow: 'hidden' },
  timeText: { fontSize: 11, color: '#9ca3af' },
  preview: { fontSize: 14, color: '#374151', lineHeight: 20, marginBottom: 12 },
  mentionLink: { color: '#22c55e', fontWeight: '500' },
  footer: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingTop: 8, borderTopWidth: 1, borderTopColor: '#f3f4f6' },
  categoryBadge: { backgroundColor: '#22c55e', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 20 },
