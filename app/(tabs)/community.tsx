...
interface VideoPlayerItemProps {
  ...
  onUnregisterPlayer: () => void;
  mediaStyle: any;
}

const VideoPlayerItem: React.FC<VideoPlayerItemProps> = ({
  url,
  isMuted,
  onRegisterPlayer,
  onUnregisterPlayer,
  ...
  useEffect(() => {
    ...
    return () => onUnregisterPlayer();
  }, [player, onRegisterPlayer, onUnregisterPlayer]);
  ...
  return (
    <VideoView
      player={player}
      style={[styles.mediaVideo, mediaStyle]}
      contentFit="contain"
                  ...
                  >
                    <View style={styles.videoWrapper}>
                      <VideoPlayerItem
                        url={item.url}
                        isMuted={muted}
                        mediaStyle={styles.mediaVideo}
                        onRegisterPlayer={(player) => {
                          if (mi === 0) videoRefs.current.set(post.id, player);
                        }}
                        onUnregisterPlayer={() => {
                          if (mi === 0) videoRefs.current.delete(post.id);
                        }}
                      />
                    </View>
                    {showControls && (
