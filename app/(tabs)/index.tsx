...
export function FarmLinkPage() {
  ...
  // Trefle states
  const [trefleSuggestions, setTrefleSuggestions] = useState<
    TrefleSearchResult[]
  >([]);
  ...
  const sanitiseTreflePlant = (plant: any): TrefleSearchResult => ({
    ...
    scientific_name:
      typeof plant.scientific_name === "string" ? plant.scientific_name : "",
    slug: typeof plant.slug === "string" ? plant.slug : "",
    image_url: typeof plant.image_url === "string" ? plant.image_url : "",
    family_common_name:
      typeof plant.family_common_name === "string"
  ...
  const searchTrefle = async (query: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `${TREFLE_PROXY_URL}?type=search&q=${encodeURIComponent(query)}`,
        { headers },
      );
  ...
  const getTrefleDetail = async (slug: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `${TREFLE_PROXY_URL}?type=detail&slug=${slug}`,
        { headers },
      );
      if (!response.ok) throw new Error(`HTTP Error ${response.status}`);
      const json = await response.json();
      if (json.data && typeof json.data === "object") {
        ...
        setTrefleDetail({
          ...
          description:
            typeof plant.species_description === "string"
              ? plant.species_description
  ...
  const analyzeWithRealAI = async (
    ...
    try {
      ...
      console.warn(
        "Gradio server unreachable. Using fallback AI result:",
        error,
      );
      ...
      return {
        ...
        preventiveTips:
          "Ensure leaves remain dry when watering to prevent future fungal infections.",
      };
    }
  };
  ...
  const openImagePicker = async (useCamera: boolean) => {
    ...
    if (!plantName.trim()) {
      Alert.alert(
        "Missing Plant Name",
        "Please enter the plant name before scanning.",
      );
      return;
    }
    let result;
    ...
    if (!result.canceled && result.assets?.length) {
      ...
      try {
        ...
        Alert.alert(
          "Error",
          "Failed to analyze plant. Is the Gradio server running?",
        );
      } finally {
    ...
    <SafeAreaView style={styles.safeArea} edges={["top", "left", "right"]}>
      <View style={styles.container}>
        ...
        >
          ...
          >
            ...
            <Text style={styles.heroSubtitle}>
              Scan your plants to detect health issues, get solutions, and track
              history
            </Text>
          </Animated.View>
          ...
          >
            <View style={styles.historyHeader}>
              ...
              <Text style={styles.historySubtitle}>
                Track your crop health and see how other farmers use AI
              </Text>
            </View>
                ...
                {scanHistory.map((scan, idx) => (
                  ...
                  >
                    ...
                    <View style={styles.cardContent}>
                      ...
                      <Text style={styles.cardCause} numberOfLines={1}>
                        {scan.analysis.cause}
                      </Text>
                    </View>
                  </Animated.View>
              ...
              <>
                <View style={styles.emptyStateContainer}>
                  ...
                  <Text style={styles.emptyStateTitle}>
                    Be the first on your farm to scan a plant 🌱
                  </Text>
                  <TouchableOpacity
                ...
                <View style={styles.communityHeader}>
                  <Text style={styles.communityTitle}>
                    How other farmers are using FarmLink
                  </Text>
                </View>
                {loadingCommunity ? (
                  <ActivityIndicator
        ...
        <Modal
          ...
          animationType="slide"
          transparent={true}
          onRequestClose={resetPlantModal}
        >
                ...
                {!selectedImage && !isAnalyzing && !analysisResult && (
                  <View style={styles.nameInputContainer}>
                    <Text style={styles.nameInputLabel}>
                      🌱 What plant are you scanning?
                    </Text>
                    <Text style={styles.nameInputSubtitle}>
                      Enter the crop or plant name. The AI will focus only on
                      this plant.
                    </Text>
                    <TextInput
                      ...
                      <View style={styles.suggestionsContainer}>
                        {trefleSuggestions.map((item) => (
                          <TouchableOpacity
                            ...
                            onPress={() => selectTrefleSuggestion(item)}
                          >
                            ...
                            <Text style={styles.suggestionFamily}>
                    ...
                    {trefleDetail && (
                      <View style={styles.trefleDetailCard}>
                        {trefleDetail.image ? (
                          <Image
                            source={{ uri: trefleDetail.image }}
                            style={styles.trefleDetailImage}
                          />
                        ) : null}
                        <Text style={styles.trefleDetailTitle}>
                          {trefleDetail.scientific_name}
                        </Text>
                        <Text style={styles.trefleDetailFamily}>
                          {trefleDetail.family}
                        </Text>
                        <Text style={styles.trefleDetailDesc} numberOfLines={3}>
                          {trefleDetail.description}
                        </Text>
                      </View>
                    )}
                    {plantName.trim() !== "" && (
                      <View style={styles.targetPlantBadge}>
                        <Ionicons
                          ...
                          color="#22c55e"
                        />
                        <Text style={styles.targetPlantText}>
                          Scanning for: {plantName.trim()}
                        </Text>
                      </View>
                    )}
                    <View style={styles.pickerContainer}>
                      <TouchableOpacity
                        style={[
                          styles.pickerButton,
                          !plantName.trim() && styles.pickerButtonDisabled,
                        ]}
                      ...
                      >
                        <Ionicons
                          name="camera"
                          size={32}
                          color={plantName.trim() ? "#22c55e" : "#9ca3af"}
                        />
                        <Text
                          style={[
