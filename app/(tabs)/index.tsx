...
const MediaViewer: React.FC<MediaViewerProps> = ({ visible, mediaItems, initialIndex, onClose }) => {
      ...
      <View style={styles.mediaViewerContainer}>
          ...
          <View style={styles.mediaViewerDots}>
            {mediaItems.map((_, i) => (
              <TouchableOpacity key={i} onPress={() => setCurrentIndex(i)}>
                <View style={[styles.mediaViewerDot, i === currentIndex && styles.mediaViewerDotActive]} />
              </TouchableOpacity>
            ))}
          </View>
        )}
        ...
        {mediaItems.length > 1 && (
          <View style={styles.mediaViewerNav}>
            <TouchableOpacity
              style={[styles.mediaViewerArrow, currentIndex === 0 && styles.mediaViewerArrowDisabled]}
              onPress={() => currentIndex > 0 && setCurrentIndex(currentIndex - 1)}
              disabled={currentIndex === 0}
            >
              <Ionicons name="chevron-back" size={32} color={currentIndex === 0 ? '#555' : '#fff'} />
            </TouchableOpacity>
            <TouchableOpacity
              style={[styles.mediaViewerArrow, currentIndex >= mediaItems.length - 1 && styles.mediaViewerArrowDisabled]}
              onPress={() => currentIndex < mediaItems.length - 1 && setCurrentIndex(currentIndex + 1)}
              disabled={currentIndex >= mediaItems.length - 1}
            >
              <Ionicons name="chevron-forward" size={32} color={currentIndex >= mediaItems.length - 1 ? '#555' : '#fff'} />
            </TouchableOpacity>
          </View>
        )}
      </View>
...
// ---------- Post Card (FIXED video auto‑play & controls) ----------
...
const PostCard: React.FC<PostCardProps> = ({
  post, onLike, onComment, onOpenChat, onReport, onDelete, onMediaPress,
  ...
  // load and play helper – waits for ref to be available
  const loadAndPlayVideo = useCallback(async () => {
    const video = videoRefs.current.get(post.id);
    if (!video) return; // not mounted yet
    try {
      const status = await video.getStatusAsync();
      if (!status.isLoaded) {
        await video.loadAsync({ uri: post.media[0].url }, { shouldPlay: true, isLooping: true, isMuted: muted });
      } else {
  ...
  useEffect(() => {
    if (visibleVideoId === post.id) {
      // Small delay to ensure the Video ref is set
      const timer = setTimeout(() => {
        loadAndPlayVideo();
      }, 100);
      return () => clearTimeout(timer);
    } else {
      // Stop & release when not visible
      const video = videoRefs.current.get(post.id);
      if (video) {
    ...
    <Animated.View entering={FadeIn.delay(index * 60)} style={styles.card}>
            ...
            renderItem={({ item, index: mi }) => {
              if (item.type === 'video') {
                return (
                  ...
                  >
                    {/* Video element always present – ref is set via callback */}
                        ...
                        onPlaybackStatusUpdate={(status) => {
                          if (status.isLoaded) {
                            ...
                            if (status.didJustFinish) {
                              videoRefs.current.get(post.id)?.setStatusAsync({ shouldPlay: false, positionMillis: 0 });
                            }
                          }
                        }}
                      />
                    ...
                    {/* Play/Pause overlay (always visible, toggles on tap) */}
                    ...
                    {/* Mute/Unmute + fullscreen (visible when controls shown) */}
                    {showControls && (
      ...
      <View style={styles.reactionSummary}>
        ...
        <Text style={styles.reactionComments}>
          {post.replies > 0 ? `${post.replies} ${post.replies === 1 ? 'comment' : 'comments'}` : ''}
        </Text>
      </View>
...
// ---------- Main Community Content (with DM fixes) ----------
const CommunityPageContent = () => {
  ...
  const requireAuth = (action?: string) => {
    if (!user) {
      Alert.alert(
        'Create an account',
        action
          ? `Please create an account to ${action} and unlock all features.`
          : 'Please create an account to access more features like posting, commenting, chatting, and applying for tenders.',
        [
  ...
  // Priority algorithm for promoted products
  ...
  const fetchPromotedProducts = async () => {
    ...
    // 3. Fetch users separately (avoids the 400 on foreign-key resolution)
    ...
    // 4. Attach seller info to each ad
    const adsWithSeller = ads.map((ad) => ({
      ...ad,
      seller: userMap[ad.user_id] || { username: 'Unknown', avatar: '🌾' },
    }));

    // 5. Sort by plan priority (desc), then by date (newest first)
  ...
  const openChatWithUser = async (userId: string, name: string, avatar: string, initialMessage?: string) => {
    ...
    try {
      // Use ChatContext if available, otherwise fallback to direct Supabase insert
      let chatId: string | null = null;
      if (typeof createChat === 'function') {
        try {
          chatId = await createChat(userId, name, avatar, initialMessage);
        } catch (ctxErr) {
          console.warn('createChat from context failed, falling back to Supabase', ctxErr);
        }
      }
      if (!chatId) {
        // Fallback: create chat directly in Supabase
      ...
      if (chatId) {
        ...
        setShowChatList(false); // close the list overlay
      }
    } catch (err: any) {
  ...
  const handleChatClose = () => {
    ...
    setShowChatList(true); // re‑open the chat list after closing a conversation
  };
    ...
    <View style={styles.safeArea}>
      <SafeAreaView style={{ flex: 1 }} edges={['top', 'left', 'right']}>
        ...
        <View style={styles.fabContainer}>
          <TouchableOpacity style={[styles.fab, styles.aiFab]} onPress={() => router.push('/(tabs)/ai_scan_index')}>
          ...
          <TouchableOpacity style={styles.fab} onPress={() => {
            ...
            {unreadMessages > 0 && (
              <View style={styles.unreadBadge}>
                <Text style={styles.unreadBadgeText}>{unreadMessages > 99 ? '99+' : unreadMessages}</Text>
              </View>
            )}
          </TouchableOpacity>
        </View>
...
// ---------- Styles (unchanged from previous full version) ----------
const styles = StyleSheet.create({
  ...
  fixedHeader: { position: 'absolute', paddingTop: 0, top: 0, left: 0, right: 0, zIndex: 10, backgroundColor: '#f0fdf4' },
  notifBtn: { padding: 8, position: 'relative' },
  notifBadge: { position: 'absolute', top: 2, right: 2, backgroundColor: '#3b82f6', borderRadius: 9, minWidth: 18, height: 18, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 4, borderWidth: 1.5, borderColor: '#fff' },
  notifBadgeText: { color: '#fff', fontSize: 10, fontWeight: '700', textAlign: 'center' },
  ...
  mediaItem: { justifyContent: 'center', alignItems: 'center', backgroundColor: '#000' },
  ...
  playPauseButton: { backgroundColor: 'rgba(0,0,0,0.5)', borderRadius: 30, padding: 12 },
  videoControlBar: { position: 'absolute', bottom: 12, left: 12, right: 12, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', },
  muteButton: { backgroundColor: 'rgba(0,0,0,0.5)', borderRadius: 20, padding: 8 },
  fullscreenButton: { backgroundColor: 'rgba(0,0,0,0.5)', borderRadius: 20, padding: 8, },
  ...
  // Full-screen media viewer styles
  mediaViewerContainer: { flex: 1, backgroundColor: '#000' },
  mediaViewerHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, paddingVertical: 8, position: 'absolute', top: 0, left: 0, right: 0, zIndex: 10 },
  mediaViewerCloseBtn: { width: 44, height: 44, borderRadius: 22, backgroundColor: 'rgba(0,0,0,0.5)', alignItems: 'center', justifyContent: 'center' },
  mediaViewerCounter: { color: '#fff', fontSize: 14, fontWeight: '600' },
  mediaViewerContent: { flex: 1, justifyContent: 'center', alignItems: 'center' },
  mediaViewerVideoWrap: { justifyContent: 'center', alignItems: 'center' },
  mediaViewerDots: { flexDirection: 'row', justifyContent: 'center', paddingVertical: 16, position: 'absolute', bottom: 80, left: 0, right: 0 },
  mediaViewerDot: { width: 8, height: 8, borderRadius: 4, backgroundColor: 'rgba(255,255,255,0.4)', marginHorizontal: 4 },
  mediaViewerDotActive: { backgroundColor: '#fff', width: 20 },
  mediaViewerNav: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 16, position: 'absolute', top: '40%', left: 0, right: 0 },
  mediaViewerArrow: { width: 44, height: 44, borderRadius: 22, backgroundColor: 'rgba(0,0,0,0.4)', alignItems: 'center', justifyContent: 'center' },
