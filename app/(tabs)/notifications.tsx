...
const MIN_TRUST_SCORE_FOR_SUBMISSION = 35;   // lowered for better UX
...
// ---------- Outbreak Report Form (with fixes) ----------
>>>>>>> gozilethu/farmlink-Mbutho
...
=======
        navigator.geolocation.getCurrentPosition(
          ...
          () => {
            setLocationError('Location required – please enable GPS in your browser.');
            setLocationLoading(false);
          }
        );
>>>>>>> gozilethu/farmlink-Mbutho
...
=======
        setLocationError('Location permission denied. GPS is required.');
>>>>>>> gozilethu/farmlink-Mbutho
        setLocationLoading(false);
        return;
      }
...
=======
  // --- Forensic helpers ---
  const getFileAgeDays = async (asset: ImagePicker.ImagePickerAsset): Promise<number> => {
    try {
      let captureDate: Date | null = null;
      if (asset.type === 'image' && asset.exif) {
        const exifDate = asset.exif.DateTimeOriginal || asset.exif.DateTimeDigitized || asset.exif.DateTime;
        if (exifDate) {
      ...
      if (!captureDate) {
        ...
        if (info.exists && info.modificationTime) captureDate = new Date(info.modificationTime * 1000);
      }
  ...
  const isSuspiciousMedia = (asset: ImagePicker.ImagePickerAsset): boolean => {
    ...
    // only flag if clearly artificial – no longer block on missing EXIF alone
    return false;
  };

  const checkAIImage = async (asset: ImagePicker.ImagePickerAsset): Promise<{ aiProbability: number; message?: string }> => {
    try {
      if (!SIGHTENGINE_API_USER || !SIGHTENGINE_API_SECRET) {
      ...
      formData.append('media', { uri: asset.uri, type: asset.type === 'image' ? 'image/jpeg' : 'video/mp4', name: 'evidence.jpg' } as any);
      ...
      return { aiProbability: aiScore, message: aiScore > 0.7 ? 'AI-generated content detected' : undefined };
    } catch (e) {
  ...
  const checkDuplicate = async (asset: ImagePicker.ImagePickerAsset): Promise<boolean> => {
    try {
      const info = await FileSystem.getInfoAsync(asset.uri, { md5: true });
      if (!info.exists || !info.md5) return false;
      const { data } = await supabase.from('evidence_hashes').select('hash').eq('hash', info.md5).maybeSingle();
      return !!data;
    } catch {
      return false;
    }
  ...
  const saveHash = async (asset: ImagePicker.ImagePickerAsset) => {
    try {
      ...
      if (info.exists && info.md5) {
        await supabase.from('evidence_hashes').upsert({ hash: info.md5, created_at: new Date() });
      }
    } catch { /* ignore */ }
  };

  // --- Fixed Trust Score (less strict) ---
  const calculateTrustScore = async (assets: ImagePicker.ImagePickerAsset[]): Promise<number> => {
    let score = 100;
    for (const asset of assets) {
      const age = await getFileAgeDays(asset);
      if (age > MAX_EVIDENCE_AGE_DAYS) score -= 15;
      ...
      } else if (aiProbability > 0.4) score -= 10;

      if (isSuspiciousMedia(asset)) score -= 10;
  ...
  const runVerification = useCallback(async (assets: ImagePicker.ImagePickerAsset[]) => {
    if (assets.length === 0) {
  ...
  // --- Fixed upload (using supabase SDK) ---
  const uploadEvidence = async (uri: string, type: 'image' | 'video'): Promise<string> => {
    const fileExt = uri.split('.').pop()?.toLowerCase() || (type === 'image' ? 'jpg' : 'mp4');
    ...
    const { data } = supabase.storage.from('farmlink').getPublicUrl(filePath);
  ...
  const handleSubmit = async () => {
    ...
    if (!evidenceAuthenticated) {
      Alert.alert('Evidence not verified', `Trust score must be at least ${MIN_TRUST_SCORE_FOR_SUBMISSION}%.`);
      return;
    }
    if (!locationName.trim()) {
      Alert.alert('Location required', 'Please enter the location (e.g., Nelspruit, Mpumalanga).');
      return;
    }
    if (!gpsCoords) {
      Alert.alert('GPS required', 'GPS coordinates are mandatory for outbreak reports.');
      return;
    }
    if (reportCountToday >= MAX_REPORTS_PER_DAY) {
    ...
    setAiChecking(true);
    try {
      const isAiGenerated = await checkAIContent(`${diseaseName} ${description}`);
      if (isAiGenerated) {
...
=======
          ...
          gps_verified: true,
          metadata_verified: verificationSteps.every(s => s.status !== 'fail'),  // softer check
...
=======
      if (error) {
        for (const url of evidenceUrls) await deleteUploadedFile(url);
        throw error;
      }
...
=======
    ...
    >
      ...
      <TouchableOpacity style={reportStyles.input} onPress={() => setShowAnimalPicker(true)}>
        <Text style={{ color: animalType ? '#111827' : '#9ca3af' }}>{animalType || 'Tap to select animal'}</Text>
      </TouchableOpacity>

      <Text style={reportStyles.label}>Disease Name *</Text>
      <TextInput style={reportStyles.input} placeholder="e.g., Foot and Mouth Disease" placeholderTextColor="#9ca3af" value={diseaseName} onChangeText={setDiseaseName} maxLength={80} />

      <Text style={reportStyles.label}>Description *</Text>
      <TextInput style={[reportStyles.input, reportStyles.textArea]} multiline textAlignVertical="top" placeholder="Describe symptoms, how many animals affected, etc." placeholderTextColor="#9ca3af" value={description} onChangeText={setDescription} maxLength={500} />
      ...
      <View style={reportStyles.card}>
        ...
        <View style={reportStyles.mediaRow}>
          {mediaItems.map((asset, idx) => (
            <View key={idx} style={reportStyles.mediaThumb}>
              <Image source={{ uri: asset.uri }} style={reportStyles.thumbImage} resizeMode="cover" />
>>>>>>> gozilethu/farmlink-Mbutho
...
=======
        ...
        <View style={[reportStyles.card, { marginTop: 16 }]}>
          ...
          {verificationSteps.map((step, idx) => (
            <View key={idx} style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 10 }}>
              {step.status === 'running' ? <ActivityIndicator size="small" color="#22c55e" style={{ marginRight: 10 }} /> :
               step.status === 'pass' ? <Ionicons name="checkmark-circle" size={18} color="#22c55e" style={{ marginRight: 10 }} /> :
               step.status === 'fail' ? <Ionicons name="close-circle" size={18} color="#ef4444" style={{ marginRight: 10 }} /> :
               <View style={{ width: 18, marginRight: 10 }} />}
              <Text style={{ flex: 1, fontSize: 14, color: step.status === 'fail' ? '#ef4444' : '#374151' }}>
          ...
          {trustScore !== null && (
            <View style={{ marginTop: 8 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                <Ionicons name={trustScore >= MIN_TRUST_SCORE_FOR_SUBMISSION ? "shield-checkmark" : "warning"} size={20} color={trustScore >= MIN_TRUST_SCORE_FOR_SUBMISSION ? '#22c55e' : '#f97316'} style={{ marginRight: 8 }} />
                <Text style={{ fontSize: 15, fontWeight: '600', color: '#111827' }}>
                  Trust Score: {trustScore}% — {trustScore >= 90 ? 'Verified Authentic' : trustScore >= 70 ? 'Likely Authentic' : trustScore >= MIN_TRUST_SCORE_FOR_SUBMISSION ? 'Suspicious' : 'Rejected'}
                </Text>
              </View>
              {trustScore < MIN_TRUST_SCORE_FOR_SUBMISSION && (
                <Text style={{ color: '#ef4444', fontSize: 13, marginTop: 4 }}>Submission requires at least {MIN_TRUST_SCORE_FOR_SUBMISSION}% trust.</Text>
              )}
            </View>
          )}
          {verificationFailed && (
            <View style={{ marginTop: 8, padding: 10, backgroundColor: '#fef2f2', borderRadius: 8 }}>
              <Text style={{ color: '#ef4444', fontWeight: '500' }}>❌ {verificationMessage}</Text>
              <Text style={{ color: '#6b7280', fontSize: 12, marginTop: 4 }}>Remove this evidence and upload recent, authentic media.</Text>
            </View>
          )}
        </View>
      )}
      ...
      <View style={reportStyles.card}>
        ...
        <Text style={reportStyles.hint}>Enter your town or farm name (e.g., Kimberley, Northern Cape)</Text>
        <TextInput style={reportStyles.input} placeholder="e.g., Polokwane, Limpopo" placeholderTextColor="#9ca3af" value={locationName} onChangeText={setLocationName} />
        <View style={{ marginTop: 12, flexDirection: 'row', alignItems: 'center' }}>
          <Ionicons name={gpsCoords ? "checkmark-circle" : locationLoading ? "time-outline" : "warning-outline"} size={16} color={gpsCoords ? '#22c55e' : '#f97316'} />
          <Text style={{ marginLeft: 6, fontSize: 13, color: '#6b7280' }}>
            {gpsCoords ? 'GPS coordinates acquired.' : locationLoading ? 'Acquiring GPS…' : 'GPS is mandatory. Enable location services.'}
          </Text>
>>>>>>> gozilethu/farmlink-Mbutho
        </View>
      </View>
...
=======
      ...
      <TouchableOpacity
        style={[
          reportStyles.submitButton,
          (!animalType || diseaseName.length < MIN_DISEASE_LENGTH || description.length < MIN_DESCRIPTION_LENGTH || mediaItems.length === 0 || submitting || aiChecking || verifying || verificationFailed || !evidenceAuthenticated) &&
        ...
        {(submitting || aiChecking || verifying) ? (
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
            ...
            <Text style={reportStyles.submitText}>
              {verifying ? 'Verifying evidence...' : aiChecking ? 'Checking authenticity...' : 'Submitting...'}
            </Text>
          </View>
        ) : evidenceAuthenticated ? (
...
=======
          <View style={reportStyles.pickerHeader}>
            ...
            <TextInput style={reportStyles.pickerSearchInput} placeholder="Search animals..." placeholderTextColor="#9ca3af" value={animalSearch} onChangeText={setAnimalSearch} autoFocus />
          </View>
          <FlatList data={filteredAnimals} keyExtractor={(item) => item} renderItem={({ item }) => (
            <TouchableOpacity style={reportStyles.animalItem} onPress={() => { setAnimalType(item); setShowAnimalPicker(false); setAnimalSearch(''); }}>
              <Text style={{ fontSize: 16, color: '#111827' }}>{item}</Text>
            </TouchableOpacity>
          )} />
>>>>>>> gozilethu/farmlink-Mbutho
...
function OutbreakHeatMap({ onMarkerPress }: { onMarkerPress: (marker: OutbreakMarker) => void }) {
  ...
  const fetchMarkers = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from('outbreak_reports')
        .select('id, location, disease_name, animal_type, status, created_at, evidence_urls, description, latitude, longitude')
        .order('created_at', { ascending: false });

      if (error) throw error;

      // Filter out null coordinates to avoid map crash
      const mapped = (data || [])
  ...
  if (loading) return <View style={styles.mapLoading}><ActivityIndicator size="large" color="#22c55e" /></View>;
  if (!MapView) return <WebMapFallback />;

  return (
    <MapView style={styles.map} initialRegion={{ latitude: -29.0, longitude: 24.0, latitudeDelta: 10, longitudeDelta: 10 }}>
...
// ---------- Main Notifications Page (only one default export) ----------
>>>>>>> gozilethu/farmlink-Mbutho
...
=======
  ...
  const fetchNotifications = useCallback(async () => {
    if (!user) { setNotifications([]); setError(''); return; }
    setLoading(true);
    setError('');
>>>>>>> gozilethu/farmlink-Mbutho
    try {
...
=======
        ...
        if (isOutbreak && n.action_url?.startsWith('outbreak:')) outbreakId = n.action_url.replace('outbreak:', '');
        return {
          id: n.id, type: n.type, title: n.title, message: n.message,
          time: new Date(n.created_at).toLocaleString(), read: n.read,
          actionUrl: n.action_url, iconName: isOutbreak ? 'warning' : iconName,
          iconColor: isOutbreak ? '#ef4444' : iconColor, isOutbreak, outbreakId,
        };
      });

      const sorted = formatted.sort((a, b) => a.isOutbreak && !b.isOutbreak ? -1 : !a.isOutbreak && b.isOutbreak ? 1 : 0);
...
=======
    if (!hasFetchedRef.current) { hasFetchedRef.current = true; fetchNotifications(); }
  }, [fetchNotifications]);
>>>>>>> gozilethu/farmlink-Mbutho
...
=======
        ...
        <TouchableOpacity style={[styles.tab, activeTab === 'outbreaks' && styles.activeTab]} onPress={() => setActiveTab('outbreaks')}>
>>>>>>> gozilethu/farmlink-Mbutho
...
=======
            ...
            {loading ? <ActivityIndicator size="large" color="#22c55e" style={{ marginTop: 50 }} /> :
             ...
             filteredNotifications.length === 0 ? <Animated.View entering={FadeIn} style={styles.emptyContainer}><GlassCard style={styles.emptyCard}><View style={styles.emptyIcon}><Ionicons name="notifications-outline" size={32} color="#22c55e" /></View><Text style={styles.emptyText}>{user ? 'No notifications yet' : 'Log in to see your alerts'}</Text></GlassCard></Animated.View> :
             <View style={styles.notificationsList}>
               {filteredNotifications.map((notification, index) => (
                 <Animated.View key={notification.id} entering={SlideInLeft.delay(index * 50)} exiting={SlideOutRight} layout={Layout.springify()}>
                   <TouchableOpacity onPress={() => { if (notification.isOutbreak && notification.outbreakId) openOutbreakFromNotification(notification); else markAsRead(notification.id); }} activeOpacity={0.8}>
                     <GlassCard style={[styles.notificationCard, !notification.read && styles.unreadCard]}>
                       <View style={styles.notificationContent}>
                         <View style={[styles.iconContainer, { backgroundColor: `${notification.iconColor}20` }]}>
                           <Ionicons name={notification.iconName as any} size={20} color={notification.iconColor} />
                         ...
                         <View style={styles.textContainer}>
                           ...
                           <View style={styles.actions}>
                             {!notification.read && !notification.isOutbreak && <TouchableOpacity onPress={() => markAsRead(notification.id)}><Text style={styles.actionText}>Mark as read</Text></TouchableOpacity>}
                             {notification.isOutbreak ? <TouchableOpacity onPress={() => openOutbreakFromNotification(notification)}><Text style={styles.actionText}>View Details</Text></TouchableOpacity> :
                              notification.actionUrl ? <Link href={notification.actionUrl as any} asChild><TouchableOpacity><Text style={styles.actionText}>View</Text></TouchableOpacity></Link> : null}
                             <TouchableOpacity onPress={() => deleteNotification(notification.id)} style={styles.deleteButton}><Ionicons name="trash-outline" size={16} color="#9ca3af" /></TouchableOpacity>
                           </View>
                         </View>
                       </View>
                       {notification.isOutbreak && <View style={styles.outbreakBadge}><Ionicons name="warning" size={12} color="#fff" /><Text style={styles.outbreakBadgeText}>OUTBREAK</Text></View>}
                     </GlassCard>
                   </TouchableOpacity>
                 </Animated.View>
               ))}
          ...
          <View style={styles.outbreakContainer}>
            <OutbreakHeatMap onMarkerPress={(marker) => { setSelectedMarker(marker); setShowDetailModal(true); }} />
            <TouchableOpacity style={styles.reportButton} onPress={() => setShowReportModal(true)} activeOpacity={0.8}>
>>>>>>> gozilethu/farmlink-Mbutho
...
<<<<<<< HEAD
      ...
      >
=======
      <OutbreakDetailModal marker={selectedMarker} visible={showDetailModal} onClose={() => setShowDetailModal(false)} />

      <Modal visible={showReportModal} animationType="slide" presentationStyle="fullScreen" onRequestClose={() => setShowReportModal(false)}>
>>>>>>> gozilethu/farmlink-Mbutho
...
=======
  tabBar: { flexDirection: 'row', backgroundColor: '#fff', marginHorizontal: 16, marginTop: 16, borderRadius: 30, borderWidth: 1, borderColor: '#e5e7eb', overflow: 'hidden' },
>>>>>>> gozilethu/farmlink-Mbutho
...
=======
  ...
  markerIcon: { width: 36, height: 36, borderRadius: 18, alignItems: 'center', justifyContent: 'center', shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.3, shadowRadius: 4, elevation: 5 },
