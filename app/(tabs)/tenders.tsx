  ...
  const [pendingAction, setPendingAction] = useState("");
   ...
   const fetchTenders = useCallback(async () => {
     try {
       ...
       let query = supabase
         .from("tenders")
         .select(
           "tender_id, title, description, product_category, quantity_needed, budget_range, delivery_location, delivery_date, deadline, status, retailer_id, created_at, tender_requirements(*)",
         )
       ...
       const retailerIds = Array.from(
         new Set(
           (tenderRows || []).map((t: any) => t.retailer_id).filter(Boolean),
         ),
       );
       ...
       if (retailerIds.length > 0) {
         const { data: retailProfileRows, error: retailProfileError } =
           await supabase
   ...
   useEffect(() => {
     if (!user) {
       ...
       setPendingAction("access tenders");
     } else {
        ...
        >
          ...
          >
            ...
            <Text style={styles.headerSubtitle}>
              Find supply contracts from retail stores
            </Text>
          </Animated.View>
            ...
            {loading ? (
              ...
              <View style={styles.emptyState}>
                ...
                <Text style={styles.emptySubtitle}>
                  Check back later for new supply opportunities
                </Text>
              </View>
            ) : (
                    ...
                    <View style={styles.tenderHeader}>
                      ...
                      <View
                        style={[
                          ...
                          {
                            backgroundColor:
                              getStatusColor(tender.status) + "20",
                          },
                        ]}
                      >
                        <Text
                      ...
                      <View style={styles.requirementsRow}>
                        {tender.requirements.slice(0, 2).map((req) => (
                          <View key={req.id} style={styles.requirementChip}>
                            <Text style={styles.requirementChipText}>
                              {req.productName}
                            </Text>
                          </View>
                        ))}
