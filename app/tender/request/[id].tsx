...
export default function ExpertRequestDetailPage() {
  ...
  const [requestStatus, setRequestStatus] =
    useState<ExpertRequestStatus>("pending");
  ...
  const handleSendReply = useCallback(async () => {
    ...
    if (!canReply) {
      return Alert.alert(
        "Not allowed",
        "Your role cannot reply to this request",
      );
    }
    ...
    try {
      ...
      const { error } = await supabase.from("expert_request_messages").insert({
        request_id: id,
        sender_id: user.id,
        content,
        is_read: false,
      ...
      if (requestStatus === "assigned") {
        ...
        if (statusErr)
          console.warn("Could not update request status", statusErr);
      }
    ...
    <SafeAreaView style={styles.safeArea} edges={["top", "left", "right"]}>
      <View style={styles.container}>
        <View style={styles.header}>
          <TouchableOpacity
            onPress={() =>
              router.canGoBack() ? router.back() : router.replace("/support")
            }
          ...
          <Animated.View entering={FadeInUp} style={styles.card}>
            <View style={styles.metaRow}>
              <View
                style={[
                  styles.statusBadge,
                  { backgroundColor: statusBg(requestStatus) },
                ]}
              >
                <Text
                  style={[
                    styles.statusText,
                    { color: statusFg(requestStatus) },
                  ]}
                >
        ...
        >
          <View style={styles.replyInner}>
            ...
            <TouchableOpacity
              style={[
                styles.sendButton,
                (!canReply || !replyText.trim() || sending) &&
                  styles.sendDisabled,
              ]}
