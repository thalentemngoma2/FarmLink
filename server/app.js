...
// Real AI analysis using Kindwise API securely from the backend
async function analyzePlant(imageBase64) {
  try {
    const response = await axios.post(
      process.env.KINDWISE_API_URL,
      {
        images: [imageBase64],
      },
      {
    ...
    // Map Kindwise response to your app's expected structure
    const topResult = response.data?.result?.disease?.suggestions?.[0];
    return {
      state: topResult?.name || "Unknown Condition",
      cause: topResult?.details?.description || "No description provided.",
      solution: topResult?.details?.treatment || "No treatment provided.",
      preventiveTips: "Refer to agricultural guidelines for prevention.",
    };
  } catch (error) {
...
app.get("/posting-server/src/index", async (req, res) => {
  ...
  let query = supabase
    .from("posts")
    .select(
      "*, profiles:user_id (full_name, avatar), comments_count, likes_count",
    )
...
app.post("/posting-server/src/index", requireAuth, async (req, res) => {
  ...
  if (!title || !preview)
    return res.status(400).json({ error: "Missing title or preview" });
...
app.get("/posting-server/src/index/:id/comments", async (req, res) => {
  ...
  const { data, error } = await supabase
    .from("comments")
    .select(
      "*, profiles:user_id (full_name, avatar), replies:comments!parent_comment_id (*, profiles:user_id (full_name, avatar))",
    )
    .eq("post_id", id)
...
app.post(
  "/posting-server/src/index/:id/like",
  requireAuth,
  async (req, res) => {
    const { id } = req.params;
...
app.post("/auth-server/src/index/signup", async (req, res) => {
  ...
  try {
    ...
    const defaultName =
      name ||
      `${email.split("@")[0]}_${uniqueSuffix}`
        .toLowerCase()
        .replace(/[^a-z0-9_]/g, "_");
    // Also generate a unique username to prevent collisions from same email prefix
    const defaultUsername = `${email.split("@")[0]}_${uniqueSuffix}`
      .toLowerCase()
      .replace(/[^a-z0-9_]/g, "_");
    ...
    res
      .status(201)
      .json({ message: "Verification email sent", user: data.user });
  } catch (err) {
...
app.get("/profile-server/src/index", requireAuth, async (req, res) => {
  const { data, error } = await supabase
    .from("profiles")
    .select(
      "full_name, email, membership_type, avatar, location, farm_size, main_crops, farming_type",
    )
    .eq("id", req.user.id)
    .single();
  if (error) return res.status(500).json({ error: error.message });
...
app.post(
  "/notification-server/src/index/:id/read",
  requireAuth,
  async (req, res) => {
    const { id } = req.params;
    const { error } = await supabase
...
app.post(
  "/notification-server/src/index/read-all",
  requireAuth,
  async (req, res) => {
    const { error } = await supabase
      .from("notifications")
...
app.delete(
  "/notification-server/src/index/:id",
  requireAuth,
  async (req, res) => {
    const { id } = req.params;
    const { error } = await supabase
...
// ==================== PLANT SCANS ====================
app.post(
  "/scan-server/src/index",
  requireAuth,
  upload.single("image"),
  async (req, res) => {
    const { plantName } = req.body;
    if (!plantName || !req.file)
      return res.status(400).json({ error: "Plant name and image required" });
    ...
    // Convert image buffer to base64 for Kindwise
    const base64Image = `data:${req.file.mimetype};base64,${req.file.buffer.toString("base64")}`;
    const analysis = await analyzePlant(base64Image);
...
app.get("/trending", async (req, res) => {
  ...
  const { data, error } = await supabase
    .from("posts")
    .select(
      "*, profiles:user_id (full_name, avatar), likes_count, comments_count",
    )
...
app.get("/tenders", async (req, res) => {
  ...
  let query = supabase
    .from("tenders")
    .select(
      "*, retail_profiles:retailer_id(store_name, city), tender_requirements(*)",
    )
    .order("created_at", { ascending: false });
...
app.get("/tenders/:id", async (req, res) => {
  ...
  const { data, error } = await supabase
    .from("tenders")
    .select(
      "*, retail_profiles:retailer_id(store_name, city, address, business_type), tender_requirements(*)",
    )
    .eq("tender_id", id)
    .single();
...
app.post("/tenders", requireAuth, async (req, res) => {
  ...
  if (!title || !description || !deadline) {
    return res
      .status(400)
      .json({ error: "Title, description, and deadline are required" });
  }
...
app.get("/my-tenders", requireAuth, async (req, res) => {
  ...
  if (role !== "retailer" && role !== "admin") {
    return res
      .status(403)
      .json({ error: "Only retailers can view their tenders" });
  }
  const { data, error } = await supabase
    .from("tenders")
...
app.post("/applications", requireAuth, async (req, res) => {
  ...
  if (error) {
    if (error.code === "23505")
      return res
        .status(409)
        .json({ error: "You have already applied to this tender" });
    return res.status(500).json({ error: error.message });
  }
...
app.get("/applications", requireAuth, async (req, res) => {
  const { data, error } = await supabase
    .from("tender_applications")
    .select(
      "*, tenders:tender_id(title, retailer_id, status, deadline, retail_profiles:retailer_id(store_name))",
    )
...
app.get("/tenders/:id/applications", requireAuth, async (req, res) => {
  ...
  if (tender.retailer_id !== req.user.id && role !== "admin") {
    return res
      .status(403)
      .json({ error: "Not authorized to view applications" });
  }
  const { data, error } = await supabase
    .from("tender_applications")
...
app.put("/applications/:id/status", requireAuth, async (req, res) => {
  ...
  const { data: tender } = await supabase
    .from("tenders")
    .select("retailer_id")
    .eq("tender_id", appData.tender_id)
    .single();
  if (tender.retailer_id !== req.user.id && role !== "admin") {
    return res.status(403).json({ error: "Not authorized" });
  }
...
app.post("/tender-messages", requireAuth, async (req, res) => {
  ...
  if (!tenderId || !receiverId || !content) {
    return res
      .status(400)
      .json({ error: "tenderId, receiverId, and content are required" });
  }
  const { data: tender } = await supabase
    .from("tenders")
    .select("retailer_id")
  ...
  if (!isRetailer && !isApplicant) {
    return res
      .status(403)
      .json({ error: "You are not a participant in this tender" });
  }
  const { data, error } = await supabase
...
app.post("/expert-requests", requireAuth, async (req, res) => {
  ...
  if (role !== "farmer" && role !== "admin") {
    return res
      .status(403)
      .json({ error: "Only farmers can send expert requests" });
  }
...
app.put("/expert-requests/:id/status", requireAuth, async (req, res) => {
  ...
  if (
    status &&
    !["pending", "assigned", "in_progress", "resolved", "closed"].includes(
      status,
    )
  ) {
    return res.status(400).json({ error: "Invalid status" });
  ...
  if (!isAssignedExpert && !isAdmin) {
    return res.status(403).json({
      error: "Only the assigned expert or admin can update this request",
    });
  }
...
app.post("/expert-requests/:id/messages", requireAuth, async (req, res) => {
  ...
  if (!isFarmer && !isExpert) {
    return res
      .status(403)
      .json({ error: "Only the farmer or assigned expert can send messages" });
  }
  ...
  if (request.status === "closed") {
    return res
      .status(400)
      .json({ error: "Cannot send messages to a closed request" });
  }
...
app.get("/expert-requests/:id/messages", requireAuth, async (req, res) => {
  ...
  if (!isFarmer && !isExpert) {
    return res
      .status(403)
      .json({ error: "Only the farmer and assigned expert can view messages" });
  }
...
app.delete("/expert-requests/:id", requireAuth, async (req, res) => {
  ...
  if (request.farmer_id !== req.user.id) {
    return res.status(403).json({
      error: "Only the farmer who created this request can delete it",
    });
  }

  if (!["pending", "assigned"].includes(request.status)) {
    return res.status(400).json({
      error: "Cannot delete a request that is in progress or resolved",
    });
  }
...
app.post(
  ...
  async (req, res) => {
    ...
    if (!animalType || !location) {
      return res
        .status(400)
        .json({ error: "Animal type and location are required" });
    }
    if (files.length === 0) {
      return res
        .status(400)
        .json({ error: "At least one photo/video is required" });
    }
...
app.get("/outbreak/verified", async (req, res) => {
  ...
  try {
    const { data, error } = await supabase
      .from("outbreak_reports")
      .select(
        "id, animal_type, disease_name, location, created_at, ai_confidence_score",
      )
      .eq("status", "verified")
      .order("created_at", { ascending: false })
      .limit(parseInt(limit));
